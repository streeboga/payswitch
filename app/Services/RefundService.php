...
final readonly class RefundService
{
    ...
    public function create(CreateRefundData $dto, int|string $merchantAccountId): Refund
    {
        ...
        $webhookEvent = DB::transaction(function () use ($refund, $payment, $result, $succeeded, $status) {
            ...
            return $this->webhookRepository->create([
                'event_type' => $succeeded ? WebhookEventType::RefundSucceeded->value : WebhookEventType::RefundFailed->value,
        ...
        // После коммита: раньше исключение ниже откатывало событие вместе с транзакцией,
        // и refund_failed не уходил никогда.
